...
<style>
    ...
    .payment-info h4 { margin: 0 0 8px 0; color: #d97706; font-size: 9pt; }
    .payment-info p { margin: 2px 0; font-size: 8pt; color: #555; }
</style>
</head>
<body>
...
<div class="header">
    <table style="width:100%"><tr>
        <td><h1>INVOICE</h1><span class="number">{{ $invoice->number }}</span></td>
        <td style="text-align:right;"><span class="badge badge-{{ $invoice->status }}">{{ ucfirst($invoice->status) }}</span></td>
    </tr></table>
</div>
...
<div>
    <div class="company-box">
        ...
        @if($invoice->company)
            ...
            @if($invoice->company->phone)<p>{{ $invoice->company->phone }}</p>@endif
        @endif
    </div>
...
<div class="meta">
    <table>
        <tr><td>Invoice Date</td><td>{{ $invoice->date->format('M d, Y') }}</td></tr>
        <tr><td>Due Date</td><td>{{ $invoice->due_date->format('M d, Y') }}</td></tr>
        <tr><td>Balance Due</td><td>${{ number_format($invoice->balance, 2) }}</td></tr>
    </table>
</div>
...
<div class="totals">
    <table>
        ...
        <tr><td>Tax ({{ $invoice->tax_rate }}%)</td><td>${{ number_format($invoice->tax_amount, 2) }}</td></tr>
        @endif
        ...
        <tr><td><strong>Balance Due</strong></td><td><strong>${{ number_format($invoice->balance, 2) }}</strong></td></tr>
        @endif
    </table>
</div>
...
<table style="width:100%; margin-top: 20px;"><tr>
    ...
    <td style="width:60%; vertical-align:top; padding: 10px 15px; background: #f9fafb; border-radius:5px; font-size: 9pt; color: #555;">
        ...
        <p style="margin:0;">{{ $invoice->company->invoice_notes }}</p>
    </td>
    @endif
    @isset($qrPath)
    <td style="width:40%; text-align:center; vertical-align:top;">
        <img src="{{ $qrPath }}" alt="QR Code" style="width: 90px; height: 90px;">
        <p style="font-size:7pt; color:#9ca3af; margin:3px 0 0;">Scan to view invoice online</p>
    </td>
    @endisset
</tr></table>
@endif
