...
<div style="padding: 4px 0; font-family: 'Segoe UI', system-ui, -apple-system, sans-serif;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; padding-bottom: 16px; border-bottom: 3px solid #f59e0b;">
        ...
        <div style="text-align: right;">
            <div style="font-size: 24px; font-weight: 800; color: #d97706; letter-spacing: -0.02em;">QUOTATION</div>
            <div style="font-size: 12px; color: #6b7280; margin-top: 3px; font-family: 'Courier New', monospace; font-weight: 600;">{{ $quotation->number }}</div>
            <div style="display: flex; align-items: center; justify-content: flex-end; gap: 8px; margin-top: 8px;">
                <span style="font-size: 11px; color: #6b7280;">{{ $quotation->date?->format('M d, Y') }}</span>
                <span style="display: inline-block; padding: 2px 12px; font-size: 10px; font-weight: 700; border-radius: 20px; background: {{ match($quotation->status) { 'draft' => '#f3f4f6', 'sent' => '#fef3c7', 'accepted' => '#ecfdf5', 'rejected' => '#fef2f2', default => '#f3f4f6' } }}; color: {{ match($quotation->status) { 'draft' => '#6b7280', 'sent' => '#d97706', 'accepted' => '#16a34a', 'rejected' => '#dc2626', default => '#6b7280' } }};">
    ...
    {{-- Bill To --}}
    <div style="padding: 14px 18px; background: linear-gradient(135deg, #fffbeb, #fff); border: 1px solid #fde68a; border-radius: 10px; margin-bottom: 20px;">
    ...
    <div style="margin-bottom: 16px;">
        <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="6" style="padding: 32px 0; text-align: center; color: #d1d5db; font-style: italic; font-size: 12px;">No items on this quotation.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div style="display: flex; justify-content: flex-end; margin-bottom: 16px;">
        <div style="width: 250px;">
            <div style="display: flex; justify-content: space-between; font-size: 12px; color: #6b7280; padding: 5px 0;">
                <span>Subtotal</span>
                <span style="color: #374151; font-weight: 500;">${{ number_format($quotation->subtotal, 2) }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 12px; color: #6b7280; padding: 5px 0;">
                <span>Tax ({{ number_format($quotation->tax_rate, 1) }}%)</span>
                <span style="color: #374151; font-weight: 500;">${{ number_format($quotation->tax_amount, 2) }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 12px; color: #6b7280; padding: 5px 0;">
                <span>Discount</span>
                <span style="color: #374151; font-weight: 500;">${{ number_format($quotation->discount, 2) }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 14px; font-weight: 700; color: #1f2937; padding-top: 10px; margin-top: 6px; border-top: 2px solid #f59e0b;">
                <span>Total</span>
                <span style="color: #d97706;">${{ number_format($quotation->total, 2) }}</span>
            </div>
        </div>
    </div>
    ...
    <div style="text-align: center; padding-top: 14px; border-top: 1px solid #f3f4f6;">
        <div style="display: inline-block; padding: 6px 20px; background: #fffbeb; border: 1px solid #fde68a; border-radius: 20px; font-size: 10px; color: #d97706; font-weight: 600;">
            Valid for 30 days &middot; Created {{ $quotation->date?->format('M d, Y') }}
        </div>
    </div>
</div>
