<div
    x-data="{
        ...
        qrData() {
            const lines = [];
            if (this.number) lines.push('Invoice: ' + this.number);
            lines.push('Amount: ' + this.formatMoney(this.total));
            if (this.paid_amount > 0) lines.push('Paid: ' + this.formatMoney(this.paid_amount));
            if (this.balance > 0) lines.push('Balance: ' + this.formatMoney(this.balance));
        ...
        cardBg() { return this.isDark ? '#374151' : '#eff6ff'; },
        barBg() { return this.isDark ? '#4b5563' : '#f3f4f6'; },
...
>
    ...
    >
        <div style="background:linear-gradient(135deg,#1e40af,#3b82f6);padding:32px 32px 24px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                ...
                <div style="text-align:right;">
                    <div style="font-size:28px;font-weight:800;color:#fff;letter-spacing:-0.03em;line-height:1;">INVOICE</div>
        ...
        <div style="padding:28px 32px 8px;display:flex;justify-content:space-between;align-items:flex-start;">
            <div>
                <div :style="'font-size:10px;font-weight:600;color:'+textTertiary()+';text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px;'">Bill To</div>
            ...
            <div style="text-align:right;">
                <div style="display:flex;gap:24px;">
                    ...
                    <div x-show="due_date">
                        <div :style="'font-size:10px;font-weight:600;color:'+textTertiary()+';text-transform:uppercase;letter-spacing:0.08em;margin-bottom:4px;'">Due</div>
                        <div :style="'font-size:13px;color:'+textPrimary()+';font-weight:500;" x-text="formatDate(due_date)"></div>
                    </div>
                </div>
                <div x-show="status" style="margin-top:12px;display:inline-flex;align-items:center;gap:6px;padding:4px 14px;border-radius:100px;font-size:12px;font-weight:600;"
                    :style="{
                        background: this.isDark ? 'rgba(255,255,255,0.1)' : (
                            status === 'draft' ? '#f3f4f6' : status === 'sent' ? '#eff6ff' : status === 'paid' ? '#f0fdf4' : status === 'overdue' ? '#fef2f2' : '#f3f4f6'
                        ),
                        color: this.isDark ? (status === 'draft' ? '#9ca3af' : status === 'sent' ? '#93c5fd' : status === 'paid' ? '#6ee7b7' : status === 'overdue' ? '#fca5a5' : '#9ca3af') : (
                            status === 'draft' ? '#6b7280' : status === 'sent' ? '#2563eb' : status === 'paid' ? '#16a34a' : status === 'overdue' ? '#dc2626' : '#6b7280'
                        )
                    }">
                    <span style="width:6px;height:6px;border-radius:50%;display:inline-block;"
                        :style="{
                            background: this.isDark ? (
                                status === 'draft' ? '#9ca3af' : status === 'sent' ? '#93c5fd' : status === 'paid' ? '#6ee7b7' : status === 'overdue' ? '#fca5a5' : '#9ca3af'
                            ) : (
                                status === 'draft' ? '#9ca3af' : status === 'sent' ? '#3b82f6' : status === 'paid' ? '#22c55e' : status === 'overdue' ? '#ef4444' : '#9ca3af'
                            )
                        }"
                    ></span>
        ...
        <div style="padding:4px 32px 24px;display:flex;gap:24px;">
            ...
            <div style="flex:1;">
                <div style="width:100%;">
                    ...
                    <div x-show="paid_amount > 0" :style="'display:flex;justify-content:space-between;padding:4px 0;font-size:13px;color:#16a34a;'">
                        <span>Paid</span>
                        <span x-text="'-'+formatMoney(paid_amount)"></span>
                    </div>
                    <div :style="'display:flex;justify-content:space-between;padding:12px 0 0;margin-top:8px;border-top:2px solid #1e40af;font-size:16px;font-weight:700;color:#1e40af;'">
                        <span x-text="paid_amount > 0 ? 'Balance Due' : 'Total Due'"></span>
                        <span x-text="paid_amount > 0 ? formatMoney(balance) : formatMoney(total)"></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div :style="'padding:16px 32px;border-top:1px solid '+borderColor()+';text-align:center;'">
            <div :style="'font-size:11px;color:'+textTertiary()+';font-weight:500;'">Thank you for your business</div>
        </div>
    </div>
    ...
    >
        <div style="text-align:center;padding:40px;">
            <div style="width:72px;height:72px;margin:0 auto 20px;border-radius:16px;background:linear-gradient(135deg,#eff6ff,#dbeafe);display:flex;align-items:center;justify-content:center;">
                <svg style="width:36px;height:36px;color:#3b82f6;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
            </div>
            <div :style="'font-size:16px;color:'+textSecondary()+';font-weight:600;'">Invoice Preview</div>
